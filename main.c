...
int add_last(void **arr, int *len, data_structure *data)
{
	...
	for (int i = 0; i < len; i++) {
		data_structure *curr = (data_structure *)last_p;
		last_p = last_p + sizeof(head) + curr->header->len;
	}
...
void print_data(void *data, u_char type) {
	...
	if (type == 2) {
		...
		int32_t *bac2 = bac1 + sizeof(int16_t);
...
void find(void *data_block, int len, int index) 
{
	...
	for (int i = 0; i < len; i++) {
		data_structure *curr = (data_structure *)print_p;
		print_p = print_p + sizeof(head) + curr->header->len;
	}

	data_structure *data_s = (data_structure *)print_p;
	print_data(print_p + sizeof(head), data_s->header->type);
}
...
int delete_at(void **arr, int *len, int index)
{
	...
	data_structure *del_s = (data_structure *)del_p;
	u_int64_t del_size = sizeof(head) + del_s->header->len;
